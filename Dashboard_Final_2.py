...
# Junta datos de establecimientos y ubigeo
...
# Elimina filas que tienen Latitud o Longitud en cero
...
if choice == "Todos":
    ...
    # Manejar el caso en que "LIMA" no esté en dist_list
    default_dist_index = dist_list.index("LIMA") if "LIMA" in dist_list else 0
    selected_dist = st.sidebar.selectbox("Distrito", dist_list, index=default_dist_index)
    ...
    # Establecer un valor predeterminado solo si "LIMA" está en la lista, de lo contrario, usar una lista vacía
    default_dist = ["LIMA"] if "LIMA" in seldist_list else []
    selected_dist = st.multiselect("Distritos", seldist_list, default=default_dist)
    with st.expander("Ver datos"):
